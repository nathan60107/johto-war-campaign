...
def generate_moves(overwrite):
    ...
    TACTICS_MOVES_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    df = read_cube(sheet_name='tactic_moves')
    for _, stats in tqdm(df.iterrows(), total=df.shape[0]):
        output_path = TACTICS_MOVES_OUTPUT_DIR / f'{stats.move_name}.png'
