...
def run():
    ...
    ABILITY_DECKS_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    for row_number, stats in tqdm(df.iterrows(), total=df.shape[0]):
        if i == 70:
            card_fronts_deck_img.save(ABILITY_DECKS_OUTPUT_DIR / CARD_FRONTS_DECK_IMG.format(j=j))
            card_fronts_deck_img = get_card_deck_base_img()
            card_backs_deck_img.save(ABILITY_DECKS_OUTPUT_DIR / CARD_BACKS_DECK_IMG.format(j=j))
            card_backs_deck_img = get_card_deck_base_img()
            i = 0
            j += 1

        pokemon_card_path = ABILITY_CARD_FRONTS_OUTPUT_DIR / f'{row_number}_{stats.ability_name}.png'
        card_back_path = ABILITY_CARD_BACKS_OUTPUT_DIR / f'{stats.ability_name}.png'
