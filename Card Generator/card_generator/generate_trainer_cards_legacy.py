...
TRAINER_CARD_ASSETS = CARD_ASSETS_DIR / 'trainer_card_assets'
...
def generate_cards(overwrite=False):
    df = read_cube(sheet_name='trainer_cards_legacy')
    for quality in QUALITIES:
        output_dir = TRAINERS_CARD_OUTPUT_DIR / quality
...
def generate_decks():
    ...
    for quality in QUALITIES:
        ...
        output_dir = TRAINERS_CARD_OUTPUT_DIR / 'decks'
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / f'{quality}_deck_legacy.png'

        for i, data in df.iterrows():
            card_path = TRAINERS_CARD_OUTPUT_DIR / quality / f'{data.trainer_class}.png'
...
def generate_deck_object():
    ...
    for i, quality in enumerate(QUALITIES):
        deck_urls[quality] = input(f'Enter the Cloud URL for {quality}_deck_legacy:\n')
        back_urls[quality] = input(f'Enter the Cloud URL for {quality}_back_legacy:\n')
    ...
    output_dir = DECK_OBJECT_OUTPUT_DIR
    output_dir.mkdir(parents=True, exist_ok=True)
    with open(output_dir / 'trainer_card_deck_legacy.json', 'w') as f:
