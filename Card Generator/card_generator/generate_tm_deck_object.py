...
def get_card_json(deck_json, i, j, stats, is_evolution=False):
    ...
    card_json['Nickname'] = stats.move_name
...
def run():
    ...
    for move_tier_filter in possible_move_tiers:
        ...
        output_path = DECK_OBJECT_OUTPUT_DIR / f'{move_tier_filter}_TM_Deck.json'
        ...
        for _, stats in df.iterrows():
            ...
            if (i == 0 and j == 0) or i == 70:
                ...
                face_url = input(f'Enter the Cloud URL for {CARD_FRONTS_DECK_IMG.format(j=j)} for {move_tier_filter}:')
                deck_json['ObjectStates'][0]['CustomDeck'][str(j + 1)]['FaceURL'] = face_url
                back_url = input(f'Enter the Cloud URL for {CARD_BACKS_DECK_IMG.format(j=j)} for {move_tier_filter}:')
