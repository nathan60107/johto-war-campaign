...
def get_card_json(deck_json, i, j, stats, is_evolution=False):
    ...
    card_json['Nickname'] = stats.tactic_name
    card_json['Tags'] = get_tags(stats)
    card_json['Hands'] = True
    card_json['Locked'] = True
    card_json['Tooltip'] = False
...
def run():
    ...
    output_path = DECK_OBJECT_OUTPUT_DIR / 'tactic_Deck.json'

    i, j = 0, 0
    for _, stats in read_cube(sheet_name='tactics').iterrows():
