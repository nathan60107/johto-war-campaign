...
def generate_cards(overwrite=False):
    df = read_cube(sheet_name='trainer_cards_2')
...
def generate_decks():
    ...
    for quality in QUALITIES:
        ...
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / f'{quality}_deck_2.png'
...
def generate_deck_object():
    ...
    for i, quality in enumerate(QUALITIES):
        deck_urls[quality] = input(f'Enter the Cloud URL for {quality}_deck_2:\n')
        back_urls[quality] = input(f'Enter the Cloud URL for {quality}_back_2:\n')
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    with open(output_dir / 'trainer_card_deck_2.json', 'w') as f:
